...
def main():
    ...
    # for curve_fit
    int_rx = list(map(int,rx))
    int_ry = list(map(int,ry))
    # print(rx,ry)
    print(int_rx,int_ry)
    rx_array, ry_array = np.array(int_rx), np.array(int_ry)
    # print(rx_array)
    popt,pcov = curve_fit(func,rx_array,ry_array)
    print(popt)

    #interpolation
    ds = 0.1
    sp = CubicSpline2D(rx, ry)
    s = np.arange(0, sp.s[-1], ds)
    ...
    for i_s in s:
        ...
        bix.append(ix)
        biy.append(iy)
        biyaw.append(sp.calc_yaw(i_s))
        bik.append(sp.calc_curvature(i_s))

    # plt.subplots(1)
    # plt.plot(bix, biy, "xb", label="Data points")
    # plt.plot(bix, biy, "-r", label="Cubic spline path")
    # plt.grid(True)
    # plt.axis("equal")
    # plt.xlabel("x[m]")
    # plt.ylabel("y[m]")
    ...
    if show_animation:  # pragma: no cover
        plt.plot(bix, biy, "xb")
        plt.plot(rx_array, func(rx_array,*popt), "k--")
        plt.plot(rx, ry, "-r")
        plt.pause(0.001)
        plt.show()
